            ...
            <CardContent className="p-6">
                <div className="flex flex-col sm:flex-row items-center sm:items-start space-y-4 sm:space-y-0 sm:space-x-6">
                    <div className="w-32 h-32 bg-background rounded-full flex items-center justify-center flex-shrink-0">
                        <img src={"/placeholder.svg"} alt={animalData.name} className="w-28 h-28 rounded-full object-cover" />
                    </div>
                    <div className="flex-1 text-center sm:text-left">
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                            <div>
                                ...
                                <p className="font-medium text-muted-foreground">{calculateAgeFloor(animalData.birthDate)}</p>
                            </div>
                            <div>
                                <p className="text-foreground">Weight</p>
                                <p className="font-medium text-muted-foreground">{animalData.weight}</p>
                            </div>
                            <div>
                                <p className="text-foreground">Location</p>
                                <p className="font-medium text-muted-foreground">{animalData.reproductiveStatus}</p>
                            </div>
                            <div>
                                <p className="text-foreground">Last Checkup</p>
                                <p className="font-medium text-muted-foreground">{animalData.name}</p>
                            </div>
                        </div>
                    </div>
                </div>
