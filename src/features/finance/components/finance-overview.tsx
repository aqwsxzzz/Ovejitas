...
import { useGetAggregateReport, useGetInventorySummaryReport, useGetMaterialConsumptionAggregateReport, useGetProfitabilityReport } from "@/features/reports/api/reports-queries";
...
export const FinanceOverview = ({ farmId, filters, assets }: FinanceOverviewProps) => {
	const dateFrom = toApiDateTime(filters.dateFrom, false);
	const dateTo = toApiDateTime(filters.dateTo, true);
	const assetsById = new Map(assets.map((asset) => [asset.id, asset]));
	const selectedAsset = filters.assetId ? assetsById.get(filters.assetId) : undefined;
	const blocksDetailedAggregates = filters.assetKind !== "all" && !filters.assetId;
	...
	const materialQuery = useGetMaterialConsumptionAggregateReport(
		{
			...
			bucket: filters.bucket,
			group_by: "material",
			material_asset_id: selectedAsset?.kind === "material" ? selectedAsset.id : undefined,
			consumer_asset_id:
				selectedAsset && selectedAsset.kind !== "material"
					? selectedAsset.id
					: undefined,
			reason: filters.materialReason === "all" ? undefined : filters.materialReason,
			date_from: dateFrom,
			date_to: dateTo,
		},
		!blocksDetailedAggregates,
	...
	const profitabilityRows = (profitabilityQuery.data?.data ?? []).filter((row) =>
	...
	const summaryByCurrency = Object.values(
		profitabilityRows.reduce(
			...
			{} as Record<string, { currency: string; income: number; expense: number; net: number }>,
		),
	);
	const rankingRows = [...profitabilityRows].sort(
		(left, right) => parseDecimal(right.net) - parseDecimal(left.net),
	);
	const trendMap = new Map<string, Map<string, { income: number; expense: number }>>();
	...
	const trendSections = Array.from(trendMap.entries()).map(([currency, rows]) => ({
		currency,
	...
	const materialRows = [...(materialQuery.data?.totals ?? [])]
		.sort((left, right) => parseDecimal(right.total_qty) - parseDecimal(left.total_qty))
		.slice(0, 5);
	...
	return (
		<div className="space-y-4">
			<section className="grid grid-cols-1 gap-4 lg:grid-cols-3">
				{profitabilityQuery.isPending ? (
					<Card className="v2-card lg:col-span-3">
						<CardContent className="pt-6 text-sm text-muted-foreground">Cargando resumen financiero...</CardContent>
					</Card>
				) : profitabilityError ? (
					<Card className="v2-card lg:col-span-3">
						<CardContent className="pt-6 text-sm text-destructive">{profitabilityError}</CardContent>
					</Card>
				...
				<Card className="v2-card">
					...
					<CardContent className="space-y-3">
						{rankingRows.length === 0 ? (
							<p className="text-sm text-muted-foreground">Sin ranking para mostrar.</p>
						) : (
				...
				<Card className="v2-card">
					...
					<CardContent className="space-y-3">
						{inventoryQuery.isPending ? (
							...
							<p className="text-sm text-muted-foreground">No hay inventario para esta seleccion.</p>
						) : (
							inventoryRows.slice(0, 6).map((row) => (
								...
								>
									<div>
										...
										<p className="text-xs text-muted-foreground">{assetsById.get(row.asset_id)?.kind ?? "asset"}</p>
									</div>
									<p className="font-semibold">{parseDecimal(row.on_hand).toFixed(2)} {row.unit}</p>
								</div>
							))
						)}
					</CardContent>
			...
			<Card className="v2-card">
				...
				<CardContent className="space-y-3">
					{blocksDetailedAggregates ? (
						<p className="text-sm text-muted-foreground">Este bloque necesita un activo especifico cuando filtras por tipo.</p>
						...
						<p className="text-sm text-muted-foreground">No hay consumo material para el rango actual.</p>
					) : (
						materialRows.map((row) => (
							...
							>
								<div className="mb-2 flex items-center justify-between gap-3">
									<p className="font-medium">{row.group_label ?? "Sin grupo"}</p>
									<p className="text-sm font-semibold">{parseDecimal(row.total_qty).toFixed(2)} {row.unit}</p>
								</div>
								<p className="text-xs text-muted-foreground">Periodo {filters.bucket} · {filters.dateFrom} a {filters.dateTo}</p>
							</div>
						))
					)}
				</CardContent>
